...
const AdminProductsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        setProducts([]); // Si hay un error, dejamos la lista vacía.
      } finally {
        setLoading(false);
      }
    };
    
    // Solo buscamos los productos si tenemos el token
    if(token) {
        fetchProducts();
    } else {
        setLoading(false);
  ...
  const handleDelete = async (productId) => {
    if (!window.confirm('¿Estás recontra seguro? No hay vuelta atrás.')) {
      return;
    }
    try {
      ...
      if (response.ok) {
        ...
        notify('Producto eliminado para siempre.', 'success');
      } else {
        const errorData = await response.json();
